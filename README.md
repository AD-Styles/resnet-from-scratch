...
기존 방식은 네트워크가 출력 `H(x)`를 **직접** 학습합니다.
ResNet은 입력 `x`를 그대로 더해 두고, 차이만큼 — 즉 잔차 `F(x) = H(x) − x` — 만 학습합니다:
...
Plain network에서 gradient는 층을 거슬러 갈 때 가중치들의 곱이 됩니다:

```
∂L/∂x₁ = ∂L/∂y_N · W_N · W_{N-1} · ... · W_1
```

가중치가 1보다 작으면 곱이 누적되며 0으로 수렴 → **Gradient Vanishing**.

반면 ResNet의 `y = F(x) + x` 에서는:

```
∂L/∂x = ∂L/∂y · (∂F/∂x + 1)
```

**"+1"** 이 항상 살아있기 때문에 `∂F/∂x`가 어떤 값이 되어도 gradient가 0으로 수렴할 수 없습니다. 이 한 줄이 152층까지 학습 가능하게 만든 수학적 이유입니다.
...
class BasicBlock(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1):
        ...
        # 차원이 바뀔 때만 1x1 conv로 맞춤 (논문 §3.2 Option B)
    ...
    def forward(self, x):
        ...
        out = out + self.shortcut(x)        # ★ H(x) = F(x) + x
        return F.relu(out)
```
...
class PlainBlock(nn.Module):
    # conv1, bn1, conv2, bn2 — BasicBlock과 동일
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
...
> **두 블록의 유일한 차이는 `forward`의 `out + self.shortcut(x)` 한 줄**. 이 한 줄이 만드는 결과가 본 실험의 본질입니다.

### CIFAR10Net — 같은 클래스로 ResNet/Plain 둘 다 만들기

```python
plain20  = CIFAR10Net(n=3, block=PlainBlock)    # Skip Connection 없음
resnet20 = CIFAR10Net(n=3, block=BasicBlock)    # Skip Connection 있음
```

`block` 인자만 바꾸면 동일 깊이·파라미터의 두 모델이 만들어집니다.
...
논문 §3.1의 핵심 가설 — *"잔차 F(x)는 일반적으로 0에 가깝다"* — 를 검증한 자리입니다. 학습된 두 모델의 모든 BatchNorm 출력에 forward hook을 걸어 layer별 std를 측정한 뒤, 논문 Figure 7 우측 panel과 동일한 형식으로 **std 내림차순 정렬**해 두 곡선의 위치 관계를 비교했습니다.
...
def get_layer_responses(model, loader, n_batches=8):
    ...
    for i, bn in enumerate(bn_modules):
        def make_hook(idx):
            return lambda m, inp, out: stds[idx].append(out.std().item())
    ...
    return [np.mean(s) for s in stds]
```
...
python src/resnet_from_scratch.py
```

RTX 4060 기준 **약 35분** 소요 (Plain-20 + ResNet-20 각각 30 epoch).
재실행 시 `results/best_*.pt` + `history_*.json` 가 있으면 학습을 건너뛰고 figure만 즉시 재생성합니다.

논문 결과(ResNet-20 91.25%)를 정확히 재현하려면 코드 상단의 `EPOCHS = 30`을 `EPOCHS = 200`으로, `LR_MILESTONES = [15, 22]`를 `[100, 150]`으로 변경하세요 (RTX 4060 기준 약 1.5~2시간).
